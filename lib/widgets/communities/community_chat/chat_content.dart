...
class ChatContent extends StatefulWidget {
  final CommunityChat? communityChat;
...
class _ChatContentState extends State<ChatContent> {
  ...
  bool isBlocked(AuthProvider ap, CommunityChat chat) {
    for (int i = 0; i < ap.loggedInUser!.blockedUsers!.length;) {
        ...
        ? Row(
            children: [
              ...
              widget.communityChat?.senderID != authData.loggedInUser?.id
                  ? IconButton(
                      onPressed: () =>
                          showOptions(rp, authData, widget.communityChat!),
                        ...
                        child: GestureDetector(
                            onTap: () async {
                              ...
                              if (isBlocked(ap, chat) == true) {
                                await ap.unBlockUser(
                                    ap.loggedInUser?.id ?? '', chat.senderID);
                              } else {
                                await ap.blockUser(
                                    ap.loggedInUser?.id ?? '', chat.senderID);
                              }
